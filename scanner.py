...
TICKERS = {
    ...
    "🚀": ["COIN", "MSTR", "MARA", "CLSK", "SOFI", "HOOD", "CRDO", "CRCL", "AAOI", "ASX", "PL", "BKSY", "ONDS", "RCAT", "APLD", "AEHR", "AXTI", "IONQ", "RGTI", "EH", "WOLF", "TQQQ"],
    "🇹🇼": ["2330.TW", "00631L.TW"],
    "₿ ": ["BTC-USD", "ETH-BTC"]
}
...
def analyze_logic(tag, sym, df_5m, df_1d):
    # 5分K 數據 (日內)
    df_5m["MA5"]  = ta.trend.sma_indicator(df_5m["Close"], window=5)
    df_5m["MA20"] = ta.trend.sma_indicator(df_5m["Close"], window=20)
    curr_5m = df_5m.iloc[-1]
    day_open = df_5m.iloc[0]["Open"]

    # 日線數據 (波段)
    df_1d["D_MA20"] = ta.trend.sma_indicator(df_1d["Close"], window=20)
    ...
    if ta.momentum.rsi(df_5m["Close"], window=14).iloc[-1] > 75 and bias > 3.5 and curr_5m["Close"] < curr_5m["MA5"]:
